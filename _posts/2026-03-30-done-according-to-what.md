...
Except nothing came next, because nothing actually happened. No PR. No commit. No record in Linear. Just a status change authored by the thing that was supposed to do the work, about the thing that was supposed to do the work.
...
Most tools in the current ecosystem have this property whether they advertise it or not. The agent is responsible for execution and for reporting on execution, which is a subtle conflict of interest that doesn't cause problems until it does, and then causes them at the worst possible time. Linear doesn't know what happened in the repository. GitHub doesn't know what the agent put in Linear. The agent knows what it intended to do, and it's confident it did it, and that confidence flows upstream as ground truth.
...
Most systems treat completion as a message from the executor. Harness treats it as a state the system has to earn by finding the evidence. The verification rejection in the screenshot is what happens when the evidence isn't there.
